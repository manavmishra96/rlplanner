...
class WorldSetup():
    ...
    def get_map_random(self):
        ...
        self.world = world_gen(n=6, goal_length=2, num_distractor=1, distractor_length=1, seed=10)
    ...
    def get_map(self, start, goal):
        ...
        srow, scol = self.BOXES_LST[start][0]
        grow, gcol = self.BOXES_LST[goal][0]

        if self.BOXES_LST[start][1] == 'box':
            srow = srow; scol = scol - 1

        desc[srow, scol] = b"S"
        desc[grow, gcol] = b"G"
        self.curpos = [grow, gcol]
